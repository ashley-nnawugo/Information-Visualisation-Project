...
//reading in csv
d3.csv("Data/StopSearchSorted.csv", function(data) {
    //nesting data
    colour.domain(d3.keys(data[0]).filter(function (id) { return id !== 'Date';}));

    data.forEach(function(d){ d.Date = formatDate(d.Date);});
    search = data;

    //mapping colours to categories
    stopSeries = colour.domain().map(function(ethnicity){
        non_numerical = data.map(function(d){
            return {Date: d.Date, number: +d[ethnicity]};
        })
        
        filtering = non_numerical.filter (function (d) { return !isNaN(d.number)});
        return {
            name: ethnicity,
            values: filtering
        };
...
function create_graph() {
    ...
    var margin = {top: 80, bottom: 100, right: 40, left:100};
    ...
    var xScale = d3.time.scale()
                        .range([0, width])
                        .domain(d3.extent(search, function(d){ return d.Date;}));
                        
    //yScale to map the range from min to max number                    
    var yScale = d3.scale.linear()
                        .range([height, 0])
                        .domain([d3.min(stopSeries, function(d) { return d3.min(d.values, function(e) {return e.number; }); }),
                                 d3.max(stopSeries, function(d) { return d3.max(d.values, function(e) {return e.number; }); })
                                    ]);
    ...
    //Defining the line data by using xScale and yScale
    ...
    svg.append("text")
        ...
        .text("Stop and Search Statistics per Race");
    
    //Creating element for the linechart
    ...
    linechart.selectAll("circle")
                    ...
                    .attr("cy", function(d) {
                        return yScale(d.number);})
                    .attr("r", 9)
                    .style("fill", function (d, i, j) { return colour(stopSeries[j].name);})
